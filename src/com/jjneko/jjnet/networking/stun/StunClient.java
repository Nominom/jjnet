...
public class StunClient extends Thread {
	...
	private static final Logger logger = Logger.getLogger(StunClient.class
			.getName());
	...
	public StunClient(InetSocketAddress stunServer,
		...
		if (s != null && s.length() > 0) {
			try {
				...
				System.out.println("Invalid com.sun.stun.CLIENT_TIMEOUT: " + s
		...
		if (s != null && s.length() > 0) {
			try {
				...
				System.out.println("Invalid com.sun.stun.CLIENT_RETRIES: " + s
						+ ".  Defaulting to " + RETRIES);
			}
		}
	...
	private void sendStunRequest() throws IOException {
		...
		logger.fine("StunClient:  asking STUN server "
				...
				+ stunServer.getPort() + " to get mapping for "
				+ addressToMap.getHostAddress() + ":" + port);
	...
	private void waitForStunResponse() throws IOException,
			SocketTimeoutException {
		byte[] response = new byte[1000];

		/*
		 * Since ports are multiplexed with STUN and data, it is possible for a
		 * us to send a BINDING_REQUEST and not immediately get a
		 * BINDING_RESPONSE but instead get data. We will toss that data. We'll
		 * either get the right response or the receive will timeout and another
		...
		for (int i = 0; i < 50; i++) {
			...
			logger.fine("Got response!  " + length + " local addr "
					+ datagramSocket.getLocalAddress() + " local port "
					+ datagramSocket.getLocalPort());
