...
/*
 * This is a modified version of a class in jVoiceBridge software
 * 
 * */
package com.jjneko.jjnet.networking.stun;
...
public class StunServer {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					logger.warning("STUN Server:  send or received failed "
							+ e.toString());
				}
			}
		}
	...
	public void processStunRequest(DatagramChannel channel,
		...
		if ((changeRequest & StunHeader.CHANGE_IP_MASK) != 0) {
			/*
			 * Not sure we can change our IP Source Address. Just ignore the
			 * request so the client thinks it failed to get a response.
			 */
			return;
		}
		...
		/*
		 * We have been asked to change our source port. Rather than use a
		 * channel, we might as well use a socket.
		 */
	...
	/*
	 * This is called from the NIST SIP Stack UDPMessageProcessor.java to get
	 * the public address of the SIP Listening point.
	 */
		...
		try {
			...
			if (request.length >= StunHeader.STUN_HEADER_LENGTH
				...
				int port = (int) (((request[StunHeader.STUN_HEADER_LENGTH
						+ StunHeader.TLV_LENGTH + 2] << 8) & 0xff00) | (request[StunHeader.STUN_HEADER_LENGTH
			...
			logger.warning("Sent STUN Binding Response from "
					...
					+ responseSocket.getLocalPort() + " to "
					+ packet.getAddress() + ":" + packet.getPort() + s);
		} catch (IOException e) {
	...
	private byte[] getStunResponse(InetSocketAddress isa, byte[] request,
		...
		if (length < StunHeader.STUN_HEADER_LENGTH) {
			...
			logger.warning(msg);
			return getBindingErrorResponse(request, StunHeader.BAD_REQUEST, msg);
		}
	...
	private byte[] processBindingRequest(InetSocketAddress isa, byte[] request,
		...
		System.arraycopy(request, 0, response, 0, StunHeader.STUN_HEADER_LENGTH);

		response[0] = 1; // set Binding Response

		response[3] = (byte) StunHeader.TLV_LENGTH
				+ StunHeader.MAPPED_ADDRESS_LENGTH + StunHeader.TLV_LENGTH
		...
		response[StunHeader.STUN_HEADER_LENGTH + 7] = (byte) (sourcePort & 0xff);
		...
		response[StunHeader.STUN_HEADER_LENGTH + 13] = StunHeader.CHANGED_ADDRESS;

		response[StunHeader.STUN_HEADER_LENGTH + 15] = StunHeader.CHANGED_ADDRESS_LENGTH;

		response[StunHeader.STUN_HEADER_LENGTH + 17] = 1; // address family

		response[StunHeader.STUN_HEADER_LENGTH + 18] = (byte) (sourcePort >> 8);
		response[StunHeader.STUN_HEADER_LENGTH + 19] = (byte) (sourcePort & 0xff);
	...
	private byte[] getBindingErrorResponse(byte[] request, int responseCode,
		...
		response[StunHeader.STUN_HEADER_LENGTH + 2] = (byte) (responseCode >> 8);

		response[StunHeader.STUN_HEADER_LENGTH + 3] = (byte) (responseCode & 0xff);
	...
	private void test() {
		...
		try {
			packet = new DatagramPacket(request, request.length,
					InetAddress.getLocalHost(), stunServerPort);
		...
		try {
			...
			if (logger.isLoggable(Level.FINEST)) {
				StunHeader.dump(
						"Sent STUN Binding Request to " + packet.getAddress()
								+ ":" + packet.getPort(), request, 0,
						request.length);
			}
		} catch (IOException e) {
